...
export default function AwardPanel({ targetId, onClose }: Props) {
  ...
  const tierWillUnlock = newTier !== associate.currentTier;
      ...
      >
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-body font-semibold text-[#1C1917] text-lg leading-tight">{associate.displayName}</p>
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-sm text-[#A8A29E] font-body">{associate.seasonPoints} pts</span>
        ...
        {/* Mode toggle */}
        <div className="flex gap-1 p-1 bg-[#F5F3EE] rounded-xl mb-4">
          <button
            onClick={() => { setMode('award'); setSelectedTag(null); }}
            className={`flex-1 py-1.5 rounded-lg text-xs font-heading transition-colors ${
              mode === 'award' ? 'bg-white text-[#1C1917] shadow-sm' : 'text-[#A8A29E]'
            }`}
          >
            + AWARD
          </button>
          <button
            onClick={() => { setMode('deduct'); setSelectedTag(null); }}
            className={`flex-1 py-1.5 rounded-lg text-xs font-heading transition-colors ${
              mode === 'deduct' ? 'bg-white text-red-600 shadow-sm' : 'text-[#A8A29E]'
            }`}
          >
            − DEDUCT
          </button>
        ...
        <AnimatePresence mode="wait">
          {mode === 'award' ? (
            <motion.div key="award" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className={`text-sm font-heading leading-none mt-0.5 ${isSelected ? 'text-[#B08C1E]' : 'text-[#A8A29E]'}`}>
                          +{pts}
                        </p>
                      </div>
                    </button>
              ...
              {/* Optional note for standard tags */}
              {selectedTag && selectedTag !== 'custom' && (
                <div className="mb-2">
                  <input
                    ...
                    className="w-full bg-[#F5F3EE] border border-[#DDD9D2] rounded-xl px-3 py-2 text-sm text-[#1C1917] font-body outline-none focus:border-[#B08C1E]/50 placeholder:text-[#C4BEB8] transition-colors"
                  />
                </div>
              )}
                ...
                {selectedTag && (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-[10px] text-[#A8A29E] font-body mb-0.5">New total</p>
                        <p className="font-heading text-[#B08C1E] text-2xl leading-none">{newTotal}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[10px] text-[#A8A29E] font-body mb-0.5">Awarding</p>
                        <p className="font-heading text-[#1C1917] text-xl leading-none">+{ptValue}</p>
                      </div>
                    </div>
                    {tierWillUnlock ? (
                      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-2 text-xs text-[#B08C1E] font-body text-center">
                        🎉 Will unlock {newTier.toUpperCase()} tier
                      </motion.p>
                    ) : nextTier ? (
                      <p className="text-[10px] text-[#A8A29E] font-body text-center mt-1.5">
                        {nextTier.threshold - newTotal} pts to {nextTier.label}
                      </p>
                    ) : null}
                  </motion.div>
                )}
            ...
            <motion.div key="deduct" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="mb-4 px-4 py-4 bg-red-50 border border-red-100 rounded-xl">
                ...
                <div className="flex items-center justify-center gap-4">
                  <button
                    onClick={() => setDeductPts(Math.max(1, deductPts - 1))}
                    className="w-11 h-11 bg-white border border-red-200 rounded-xl text-red-500 hover:bg-red-100 transition-colors font-heading text-xl"
                  >−</button>
                  <span className="w-14 text-center font-heading text-red-600 text-4xl leading-none">{deductPts}</span>
                  <button
                    onClick={() => setDeductPts(Math.min(associate.seasonPoints || 20, deductPts + 1))}
                    className="w-11 h-11 bg-white border border-red-200 rounded-xl text-red-500 hover:bg-red-100 transition-colors font-heading text-xl"
                  >+</button>
                </div>
                <p className="text-center text-xs text-red-400 font-body mt-2">
                  {associate.seasonPoints} → {Math.max(0, associate.seasonPoints - deductPts)} season pts
                </p>
              </div>
              ...
              <div className="mb-4">
                <input
                  ...
                  placeholder="Reason (optional)"
                  className="w-full bg-[#F5F3EE] border border-[#DDD9D2] rounded-xl px-3 py-2 text-sm text-[#1C1917] font-body outline-none focus:border-red-300 placeholder:text-[#C4BEB8] transition-colors"
                />
              </div>

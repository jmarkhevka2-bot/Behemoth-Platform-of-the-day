...
const TIER_LABELS: Record<Tier, string> = {
  none:    '',
  bronze:  'BRONZE OPERATOR',
  silver:  'SILVER OPERATOR',
  gold:    'GOLD OPERATOR',
  diamond: 'DIAMOND OPERATOR',
  legend:  'BEHEMOTH LEGEND',
};
...
export default function TierCelebration({ onDone }: Props) {
  ...
  useEffect(() => {
    ...
    const t3 = setTimeout(() => {
      dispatch({ type: 'CLEAR_PENDING_CELEBRATION' });
      onDone();
    }, 5500);
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[60] bg-[#F5F3EE] flex flex-col items-center justify-center"
    >
      {/* Subtle radial glow behind badge */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: `radial-gradient(ellipse 60% 50% at 50% 45%, ${accentColor}18 0%, transparent 70%)`,
        }}
      />
      ...
      <div className="relative flex flex-col items-center text-center px-6">
        ...
        <motion.p
          ...
          className="font-heading text-5xl sm:text-7xl text-[#1C1917] mb-2"
        >
        ...
        <motion.p
          ...
          className="font-heading text-2xl sm:text-3xl tracking-widest"
        ...
        {tierConfig && (
          <motion.div
            ...
            className="mt-5 p-4 bg-white border border-[#DDD9D2] rounded-2xl max-w-sm shadow-sm"
          >
            <p className="text-[10px] text-[#A8A29E] font-body mb-1 uppercase tracking-wider">Reward unlocked</p>
            <p className="text-[#6B6560] font-body text-sm leading-snug">{tierConfig.reward}</p>
          </motion.div>
        )}
        ...
        <motion.button
          ...
          className="mt-8 px-8 py-3 border border-[#DDD9D2] text-[#6B6560] font-heading text-sm rounded-xl hover:bg-white transition-colors"
        >
          CELEBRATE! 🎉
        </motion.button>
      </div>

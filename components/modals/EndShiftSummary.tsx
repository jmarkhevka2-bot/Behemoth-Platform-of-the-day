...
export default function EndShiftSummary({ onClose, onStartNew }: Props) {
  ...
  function handleEndAndReset() { dispatch({ type: 'RESET_DAILY' }); onClose(); }
  function handleStartNew()    { dispatch({ type: 'RESET_DAILY' }); onStartNew(); onClose(); }
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ y: 24, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 24, opacity: 0 }}
        ...
        <div className="px-5 py-4 border-t border-[var(--border-subtle)] flex gap-3">
          <button onClick={handleEndAndReset}
            className="flex-1 py-2.5 border border-[var(--border)] text-[var(--text-secondary)] text-sm font-body rounded-xl hover:bg-[var(--bg-secondary)] transition-colors">
            Close & Reset Daily
          </button>
